...
// MD5 hash of file content below this line: 85949a386e2e3069f81e07f6aaed1a94
...
py::dict parse_recipe_2ebe225db0430fcf_istream(std::istream& cont, ParsingOptions& parse_opts);
      ...
      if (should_parse_section(mf, mt, exclude, include)) {
        {
          ...
          curdict = curdict[py::cast(mf)];
          curdict[py::cast(mt)] = parse_recipe_2ebe225db0430fcf_istream(cont, parse_opts);
        }
      } else {
        verbatim_section = read_section_verbatim(mat, mf, mt, cont, is_firstline, parse_opts);
        {
...
py::dict parse_mf4(std::string& strcont, ParsingOptions parse_opts) {
  std::istringstream iss(strcont);
  return parse_recipe_2ebe225db0430fcf_istream(iss, parse_opts);
}
...
if ((! inpfile.is_open())) {
  ...
  return parse_recipe_2ebe225db0430fcf_istream(inpfile, parse_opts);
}
...
void write_recipe_2ebe225db0430fcf_ostream(std::ostream& cont, py::dict endf_dict, WritingOptions& write_opts);
    ...
    for (auto mt_key : sorted_mt_keys) {
      ...
      if (mf == 0 && mt == 0) {
        ...
        if (py::isinstance<py::dict>(endf_dict[py::cast(mf)][py::cast(mt)])) {
          ...
          write_recipe_2ebe225db0430fcf_ostream(cont, py::cast<py::dict>(endf_dict[py::cast(mf)][py::cast(mt)]), write_opts);
        } else {
          write_section_verbatim(cont, py::cast<py::list>(endf_dict[py::cast(mf)][py::cast(mt)]), write_opts);
        }
        mat = get_mat_from_mfmt_section(mt_dict);
...
std::string write_mf4(py::dict endf_dict, WritingOptions write_opts) {
  std::ostringstream oss;
  write_recipe_2ebe225db0430fcf_ostream(oss, endf_dict, write_opts);
  return oss.str();
}
...
void write_mf4_file(std::string& filename, py::dict endf_dict, WritingOptions write_opts) {
  ...
  write_recipe_2ebe225db0430fcf_ostream(outfile, endf_dict, write_opts);
  outfile.close();
}
